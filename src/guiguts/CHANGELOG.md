...
- Alt-text editor added using a stripped down version of the Auto-illus dialog.
  User can add/edit alt text, and change the "decorative" setting, before
  advancing to the next `img`
- PPcomp now has two modes of display controlled by radio buttons at the top of
  the dialog: Compact (as in previous versions of GG2) and Expanded (more
  similar to the online Workbench PPcomp output)
- Right-clicking on the Back/Forward buttons in the toolbar will pop a context
  menu listing the last 10 stored location history points
- `\M...\E` regex extension replaces a Roman number with a regnal string, e.g.
  "VIII" with "the Eighth" for use with monarchs, popes, etc.
- PPhtml includes title text in its listing of headings
- HTML Autogeneration adds `data-epub-type` to the autogenerated ToC and to any
  index to aid accessibility
- When a new file is loaded (or Switch File is used in PPcomp), instead of
  closing all dialogs, they now remain open, but clearing any messages relevant
  to the previously-loaded file.  
- Keyboard focus is now returned to the main text window when a status bar
  or toolbar button is pressed
- Protect HTML Straight Quotes now also applies to TeX code
- PPhtml now reports if the `<title>` contains HTML tags

### Bug fixes

- Columns did not resize well in Page Labels dialog
- Two nearby letters marked individually with small-caps markup did not apply
  markup correctly using Text->Convert Markup
- Multiple lowercase letter anchors, e.g. `[a]`, were not detected correctly
- Regex `[][]` was not reported or trapped leading to multiple messages
- HTML & CSS validator triggered exceptions and ebookmaker failed silently if
...
- New color themes have been added to the Edit-->Settings dialog due to the
  upcoming release of Tk9, which would not work with the old themes.
  A light theme and a dark theme can be chosen. The light/dark setting can
  be forced to Light, or to Dark, or to Auto, which selects light/dark based
  on the operating system setting. If a theme does not display well on your
  computer, or makes the interface seem slow to react, choose another theme.
  Several themes, including the "default" and "clam" light themes, are
  traditional themes which should work well on all platforms.
- The Regex Library "italic_semantic.json" file has been upgraded to also
  handle `<b>`-->`<strong>`, so has been renamed to "semantic_fixup.json"
- Unmatched Brackets check now has a "Restrict matches to same paragraph"
  option to give more flexibility
- "Rewrap Selection Except Indented Text" added to Tools menu, which 
  rewraps the selected text, except for indented lines
- A WAVE Report button has been added to the Ebookmaker API dialog, which
  will give a report on the most recently generated HTML file in the cache
- HTML autogeneration adds `<main>` tags around the whole book to aid
  accessibility
- HTML autogeneration adds `<nav>` tags around the autogenerated ToC to aid
  accessibility - if the auto-Toc is not used, move these tags to surround
  the actual ToC
- HTML autogeneration adds `<nav>` tags around a `/I...I/` index
- When HTML autogeneration adds `<hr>` elements between paragraphs, it now
  adds `aria-hidden="true"` so it is not announced by screen readers
- When the png/scans directory is set, the value is immediately saved to
  the project json file
- When processing page separators, all types of block markup are now tidied
  if they close immediately before, and open immediately after, the page sep
- "i.e." and "e.g." are no longer reported by "word.word" Regex Scanno
- Status bar "goto" buttons are all moved to the left, and tooltips have been
  made less confusing 
- The tab order in the Search/Replace dialog has been improved

### Bug fixes

- Jeebies failed to report phrases such as "will he here", when there were
...
- An exception could be generated if files in the GGprefs directory didn't
  have write access
- Clicking rapidly on Header/Footer Fixup, Fix & Hide button could cause
  previously hidden messages to reappear
- Word Frequency failed to find words containing a trailing apostrophe
